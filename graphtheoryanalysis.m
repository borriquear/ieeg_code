...
function [] = graphtheoryanalysis()
%% graphtheoryanalysis calculate clustering, path length ... from connectivity matrix matfile
% dependencies : need 2 mat files: EEG object and connectivity matrix 
% source folder is in patient/session
% load EEG data in EEG object using initialize_EEG_variables 
% set wavelet using initialize_wavelet

% Figure 31.4
% 1. Display chart with distribution of correlation coefficients and binarized matrix
% 2. Draw connectivity on topology map, load the coordinates of each
% electrode in EEG object
disp('Loading data from EEG .mat file....')
...
%load the connectivity matrix previously created
[pathstr, subjandcond, matext] = fileparts(myfullname);
% load connectivity matrix
corrtechniquex = {'Phase-based' 'Spearman' 'Pearson'};
connmattoload = 2; % 1,2 for power based, P or S and 3 for phase based
corrtechnique = corrtechniquex(connmattoload);
if connmattoload > 1
    typeconn = 'Power-based'
else
    typeconn = 'Phase-based'
end
%Build fnofconnmatrix from myfullname
disp(['Loading mat file of connectivity matrix...:'])
folderfig = fullfile(pathstr,subjandcond)
cd(folderfig);
%subjandcond = strcat('fig-', subjandcond);
%display the objects in the 'folderfig' folder
fileinfolder = what(folderfig);
counterctmat = size(fileinfolder.mat,1);
if counterctmat == 0
     disp(['ERROR! no .mat file of conectivity matrix found in' fnofconnmatrix])
    return
else
    disp(['Found '  num2str(counterctmat) ' .mat files of connectivity matrixes in the target directory'])
    disp(fileinfolder.mat)
end
%fnofconnmatrix = 'D:\BIAL PROJECT\patients\Way, SA\Spearman_Fq_10_EEG_cut_BL_HYP_was30_11172015_s1.mat'
idxmatconmat = fileinfolder.mat(connmattoload);
disp(['Loading connectivity matrix:'])
fmatcorr = load(idxmatconmat{1});
disp(['OK. Connectivity matrix ' idxmatconmat ' LOADED'])

connectivitymat = fmatcorr.corrmat; 
nameofcm = fmatcorr.corr_mat_name;
[mat_pathstr,mat_name,mat_ext] = fileparts(nameofcm)
%[corrtechnique remain] = strtok(nameofcm, '_');
disp('Data Loaded OK!');
%Calculate threshold
%Calculate cluster coefficient and path length

center_freq_bands = [2.5 5 10 20 40] %delta, theta, alpha,beta, gamma
center_freq_bands = [2.5]
disp(['Calculating GT parameters for ' num2str(size(center_freq_bands,2)) ' frequency bands '] )
%
for freq_index=1:size(center_freq_bands,2)
    center_freq = center_freq_bands(freq_index); %5
    disp(['Calling calculatgraphparams for frequency=' num2str(center_freq_bands(freq_index)) ] )
    [pli_thresh, binth_connectivitymat] = calculatebinarizedconnmatrix(EEG,connectivitymat,nameofcm,corrtechnique,typeconn, center_freq)
    
    calculatgraphparams(EEG, center_freq,binth_connectivitymat, pli_thresh, nameofcm,times2plot);
end
end

function [pli_thresh, binth_connectivitymat] = calculatebinarizedconnmatrix(EEG,connectivitymat,nameofcm,corrtechnique,typeconn, center_freq)
%% calculatebinarizedconnmatrix  input connectivitymat and returns threshold used and the binarized con matix
[pathd namef ] = fileparts(nameofcm);
corrtechnique = strtok(namef, '_');

disp('Calculating graph parameters ..')
figure
...
%corrtechnique = 'kk'
xlabel({[ corrtechnique ' Correlation coeff' ], ['Threshold = median +std =' num2str(pli_thresh)]}), ylabel('Count of pairs')
[mat_pathstr,mat_name,mat_ext] = fileparts(nameofcm);
if strcmp(corrtechnique,'Spearman') == 1
  [ctype remain] = strtok(mat_name,  'Spearman_');
end
if strcmp(corrtechnique,'Pearson') == 1
  [ctype remain] = strtok(mat_name,  'Pearson');
end

[freq remain] = strtok(remain, '_Fq_');
[condition remain]= strtok(remain, '_EEG_cut_');
[condition2 remain2]= strtok(remain, '_');
[patient remain3]= strtok(remain2, '_');
title([typeconn ' connectivity. Fq=' freq ',Condition-Subj =' condition condition2 '-' patient  ] );

subplot(223)
% Binarized connectivity matrix
binth_connectivitymat = connectivitymat;
binth_connectivitymat(logical(triu(binth_connectivitymat))) = 0; % eliminate lower triangle
binth_connectivitymat = binth_connectivitymat + tril(binth_connectivitymat)';  % mirror lower triangle to upper triangle
binth_connectivitymat(binth_connectivitymat < pli_thresh)=0;
binth_connectivitymat(binth_connectivitymat > 0)=1;
imagesc(logical(binth_connectivitymat)) % logical converts to 0's and 1's, thus binarizing connectivity matrix
set(gca,'clim',[0.0 1.0],'xtick',1:8:EEG.nbchan,'xticklabel',{EEG.chanlocs(2:8:end).labels},'ytick',1:8:EEG.nbchan,'yticklabel',{EEG.chanlocs(2:8:end).labels});
title([ ' binarized connectivity'  ] );
axis square
end


function [] = calculatgraphparams(EEG, center_freq,binth_connectivitymat, pli_thresh, nameofcm,times2plot)
%% calculatgraphparams  calculate graph parameters e.g. clustering coeff. path length
% for frequency center_freq and times times2plot time
% Calculate cluster coefficient
timecols = EEG.pnts;
clustcoef = zeros(1,EEG.nbchan-1);
% Calculate  connectivity degree
conndegree = zeros(1,EEG.nbchan-1);     
for chani=1:EEG.nbchan-1
    % find neighbors (suprathreshold connections)
    neighbors = find(binth_connectivitymat(chani,:));
    n = length(neighbors);
    conndegree(chani) = n;
    % cluster coefficient not computed for islands
    if n>1
        % "local" network of neighbors
        localnetwork = binth_connectivitymat(neighbors,neighbors);
...
% topoplots Topological plot of the cluster coefficient
%         figure(figh1)
...
subplot(2,2,4)
%subplot(length(freqs2plot),length(times2plot),ti+(fi-1)*length(times2plot))
plot(conndegree,clustcoef,'.')
[r,p] = corr(conndegree',clustcoef','type','s');
set(gca,'ylim',[-.05 1.05],'xlim',[0 30])



xlabel({[ 'degree, mean length path=' num2str(mean(conndegree))]}),
ylabel({['cluster coefficient'],[ 'Mean cc=' num2str(mean(clustcoef))]})


title([ 'Corr.: r=' num2str(r) ', p=' num2str(p) ])
...
plot(sum(logical(ispc_mat)),EEG.chanlocs,'plotrad',.53,'maplimits',[0 25]);

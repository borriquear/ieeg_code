...
% 
function [ ] =  graphtheorymetrics()

%% Figure 31.2

% load data
%load sampleEEGdata
[myfullname, EEG_study, channel_labels, initchan, center_freq, chantouse, time] = initialize_EEG_variables();
time2analyze  = max(EEG_study.times); % in ms
inittime2analyze = min(EEG_study.times);
endtime2analyze = round(time2analyze/EEG_study.srate);
% wavelet and FFT parameters
time          = inittime2analyze:1/EEG_study.srate:endtime2analyze;
[half_wavelet, n_wavelet, n_data n_convolution, wavelet_cycles] = initialize_wavelet(time, EEG_study);

% initialize connectivity output matrix
connectivitymat = zeros(EEG_study.nbchan,EEG_study.nbchan);

% time in indices
[junk,tidx] = min(abs(EEG_study.times-time2analyze));
...
analyticsignals = zeros(EEG_study.nbchan,EEG_study.pnts,EEG_study.trials);
initchan = 2;
for chani=initchan:EEG_study.nbchan
    
    % FFT of data
    data_fft = fft(reshape(EEG_study.data(chani,:,:),1,n_data),n_convolution);
    ...
    analyticsignals(chani,:,:) = reshape(convolution_result,EEG_study.pnts,EEG_study.trials);
end

% now compute all-to-all connectivity
connectivitymat = zeros(EEG_study.nbchan-1,EEG_study.nbchan-1);
for chani=initchan:EEG_study.nbchan
    for chanj=chani:EEG_study.nbchan % note that you don't need to start at 1
        ...
        connectivitymat(chani-1,chanj-1) = abs(mean(sign(imag(xsd))));
        connectivitymat(chanj-1,chani-1) = abs(mean(exp(1i*angle(xsd))));
    end
end

figure
imagesc(connectivitymat)
%set(gca,'clim',[0 .7],'xtick',1:8:EEG_study.nbchan,'xticklabel',{EEG_study.chanlocs(1:8:end).labels},'ytick',1:8:EEG_study.nbchan,'yticklabel',{EEG_study.chanlocs(1:8:end).labels});
xtlabels = {EEG_study.chanlocs.labels};
ytlabels = xtlabels;

set(gca,'clim',[0 1],'xtick',1:6:EEG_study.nbchan,'xticklabel',xtlabels(2:6:end),'ytick',1:6:EEG_study.nbchan,'yticklabel',ytlabels(2:6:end));
axis square
title('Connectivity matrix (phase-lag index on upper triangle; ISPC on lower triangle)')
colorbar
...
imagesc(pli_mat)
%set(gca,'clim',[0 .7],'xtick',1:8:EEG_study.nbchan,'xticklabel',{EEG_study.chanlocs(1:8:end).labels},'ytick',1:8:EEG_study.nbchan,'yticklabel',{EEG_study.chanlocs(1:8:end).labels});
set(gca,'clim',[0 1],'xtick',1:10:EEG_study.nbchan,'xticklabel',xtlabels(2:10:end),'ytick',1:6:EEG_study.nbchan,'yticklabel',ytlabels(2:6:end));
title('Thresholded PLI conn. matrix')
axis square
...
axis square
title('Thresholded ISPC conn. matrix')
%% Figure 31.6

% figure
% subplot(121)
% % note: logical() below converts the matrix to 0's and 1's
% topoplot(sum(logical(pli_mat)),EEG_study.chanlocs,'plotrad',.53,'maplimits',[0 25]);
% title('Connectivity degree, phase-lag index')
% 
% subplot(122)
% topoplot(sum(logical(ispc_mat)),EEG_study.chanlocs,'plotrad',.53,'maplimits',[0 25]);
...
% initialize matrices
alldata    = zeros(EEG_study.nbchan,length(frex),length(times2save),EEG_study.trials);
tf_all2all = zeros(EEG_study.nbchan,EEG_study.nbchan,length(frex),length(times2save));
tf_degree  = zeros(EEG_study.nbchan,length(frex),length(times2save));

% first, run convolution for all electrodes and save results
for chani=1:EEG_study.nbchan
    
    % FFT of activity at this electrode (note that 
    % this is done outside the frequency loop)
    eeg_fft = fft(reshape(EEG_study.data(chani,:,:),1,[]),n_conv2);
    ...
    for fi=1:length(frex)
        ...
        asig     = reshape(conv_res(half_wavelet+1:end-half_wavelet),EEG_study.pnts,EEG_study.trials);
...
for chani=1:EEG_study.nbchan
    for chanj=chani+1:EEG_study.nbchan
...
for fi=1:length(freqs2plot)
    for ti=1:length(times2plot)
        ...
        topoplot(squeeze(tf_degree(:,dsearchn(frex',freqs2plot(fi)),dsearchn(times2save',times2plot(ti)))),EEG_study.chanlocs,'plotrad',.53,'maplimits',clim,'electrodes','off');
...
figure
contourf(times2save,frex,squeeze(tf_degree_base(strcmpi(electrode2plot,{EEG_study.chanlocs.labels}),:,:)),20,'linecolor','none')
...
for fi=1:length(freqs2plot)
    for ti=1:length(times2plot)
        ...
        % initialize
        clustcoef = zeros(size(EEG_study.nbchan));
        ...
        figure(figh1)
        subplot(length(freqs2plot),length(times2plot),ti+(fi-1)*length(times2plot))
        %topoplot(clustcoef,EEG_study.chanlocs,'plotrad',.53,'maplimits',clim,'electrodes','off');
...
clustercoefficients = zeros(EEG_study.nbchan,length(thresholds));
...
for neti=1:10
    for probi=1:length(probs)
        ...
        clustcoef_rewired  = zeros(1,EEG_study.nbchan);
        pathlength_rewired = zeros(1,EEG_study.nbchan);
        for chani=1:EEG_study.nbchan
...
for threshi=1:length(thresholds)
    ...
    clustcoef_real = zeros(1,EEG_study.nbchan);
    for chani=1:EEG_study.nbchan
        neighbors = find(connmat(chani,:));
        n = length(neighbors);
        if n>1
    ...
    for permi=1:n_permutations
        ...
        clustcoef_random  = zeros(1,EEG_study.nbchan);
        for chani=1:EEG_study.nbchan
